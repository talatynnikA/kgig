...
// Index=0 –  Диффузная модель отражения света
// Index=1 –  Зеркальная модель отражения света
{
	...
	COLORREF Col;
	double df = 1.1;                              // Шаг по азимуту
	double dq = 1.1;                              // Шаг угла	
	double kLight;
	VSphere(0) = 3;
	...
	for (double fi = 0; fi < 360.0; fi += df)		  // Азимут
		for (double q = 0; q <= 180.0; q += dq)		  // q = 90 - угол места
		{
			...
			if (cos_RN > 0) // Если точка видима наблюдателю
			{
				...
				{		//св-ва матер. поверхн
					if (Index == 0)				///диффуз
						kLight = cos_PN * cos_RN; // cos угла падения луча на cos (в-р т.наблюд. ; в-р N)
					else if (Index == 1)		///зеркал
						kLight = pow(CosV1V2(VSphereNorm, VS), 17);// косинус между вектором нормали и источником наблюдения в декарт. в 7 степени(от1 до 200)
