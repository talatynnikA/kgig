...
void CPlot3D::CreateMatrF(double dx,double dy)//////////////получаем коордн функции
{
...
void CPlot3D:: CreateMatrView()////////////////переводим в обл видимости
{
	...
	{
		VecMatrix.clear();
		for(int j = 0;j < MatrF[i].size();j++)    //пересчет
		{
			VX = MatrF[i][j];
			VX = MV*VX;
			V(0) = VX(0);
...
void CPlot3D:: CreateMatrWindow()/////////////из мир в окон
{
